    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        for lane_index, coords in enumerate(lane_coords):
            if data_source == 'tusimple':
                ...
                tmp_mask[tuple((np.int_(coords[:, 1] * 720 / CFG.TRAIN.IMG_HEIGHT), np.int_(coords[:, 0] * 1280 /  CFG.TRAIN.IMG_WIDTH)))] = 255
            elif data_source == 'beec_ccd':
                tmp_mask = np.zeros(shape=(1350, 2448), dtype=np.uint8)
                tmp_mask[tuple((np.int_(coords[:, 1] * 1350 / CFG.TRAIN.IMG_HEIGHT), np.int_(coords[:, 0] * 2448 /  CFG.TRAIN.IMG_WIDTH)))] = 255
            else:
